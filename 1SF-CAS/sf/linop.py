...
class LinOpH (LinearOperator):
    ...
    def do_cas_1sf(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        ################################################ 
        v_b1 = np.reshape(v, (ras2,ras2)) # v for block 1
        ...
        sig_1 = np.einsum("ib,ba->ia", v_b1, Fa_tmp) 
        sig_1 = sig_1 - np.einsum("ja,ji->ia", v_b1, Fi_tmp)
        #   sig(ai:ba) += -v(bj:ba) I(ajbi:baba)
        tei_tmp = self.tei.get_subblock(2, 2, 2, 2)
        sig_1 = sig_1 - np.einsum("jb,ajbi->ia", v_b1, tei_tmp)
        # using reshape because non-contiguous in memory
        # look into this while doing speedup
        sig_1 = np.reshape(sig_1, (v.shape[0], 1))
        print(sig_1.shape)
    ...
    def do_cas_2sf(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref1 = np.zeros((ras2, ras2, ras2, ras2))
        index = 0
        for i in range(ras2):
            for j in range(i):
                for a in range(ras2):
    ...
    def do_h_1sf(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        # v(1) indexing: (ia:ab)
        v_ref1 = np.reshape(v_b1, (ras2, ras2))
        # v(2) indexing: (Ia:ab)
        v_ref2 = np.reshape(v_b2, (ras1, ras2))
        # v(3) unpack to indexing: (Iiab:babb)
        v_ref3 = np.zeros((ras1, ras2, ras2, ras2))
        index = 0
        for I in range(ras1):
            for i in range(ras2):
                for a in range(ras2):
    ...
    def do_p_1sf(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref1 = np.reshape(v_b1, (ras2, ras2))
        # v(2) indexing: (Ai:ab)
        v_ref2 = np.reshape(v_b2, (ras3, ras2))
        # v(3) unpack to indexing: (Aijb:aaab)
        v_ref3 = np.zeros((ras3, ras2, ras2, ras2))
        index = 0
        for i in range(ras2):
            for j in range(i):
                for A in range(ras3):
    ...
    def do_hp_1sf(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref2 = np.reshape(v_b2, (ras1, ras2))
        # v(3) indexing: (Ai:ab)
        v_ref3 = np.reshape(v_b3, (ras3, ras2))
        # v(4) unpack to indexing: (Iiab:babb)
        v_ref4 = np.zeros((ras1, ras2, ras2, ras2))
        index = 0
        for I in range(ras1):
            for i in range(ras2):
                for a in range(ras2):
        ...
        v_ref5 = np.zeros((ras3, ras2, ras2, ras2))
        index = 0
        for i in range(ras2):
            for j in range(i):
                for A in range(ras3):
    ...
    def do_p_ea(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref1 = np.reshape(v_b1, (ras2))
        # v(2) indexing: (A:b)
        v_ref2 = np.reshape(v_b2, (ras3))
        # v(3) indexing: (iAa:aab)
        v_ref3 = np.reshape(v_b3, (ras3, ras2, ras2))
    ...
    def do_h_ip(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref1 = np.reshape(v_b1, (ras2))
        # v(2) indexing: (I:a)
        v_ref2 = np.reshape(v_b2, (ras1))
        # v(3) indexing: (Iia:bab)
        v_ref3 = np.reshape(v_b3, (ras1, ras2, ras2))
    ...
    def do_h_1sf_ip(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref2 = np.reshape(v_b2, (ras1, ras2, ras2))
    ...
    def do_p_1sf_ea(self, v, Fa, Fb, tei, offset_v, ras1, ras2, ras3):
        ...
        v_ref2 = np.reshape(v_b2, (ras3, ras2, ras2))
