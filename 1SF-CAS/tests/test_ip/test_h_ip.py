...
def test_1():
    ...
    wfn = fock_ci( 1, 0, n2_7, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 4})
...
def test_2():
    ...
    psi4.core.clean_variables()
    options = {"basis": "cc-pvdz", 'num_roots': 4, 'diis_start': 20, 'e_convergence': 1e-10, 'd_convergence': 1e-10, 'scf_type': 'pk', 'diag_method': 'rsp'}
    expected = sf_cas_ref( 1, 4, n2_3, conf_space="h", add_opts=options )
    wfn = fock_ci( 1, 0, n2_3, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 4})
    assert abs(wfn.e[0] - expected) < threshold
...
def test_3():
    ...
    wfn = fock_ci( 1, 0, o2, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 2})
    assert abs(wfn.e[0] - expected) < threshold
