...
def test_1():
    ...
    wfn = fock_ci( 2, 2, n2_7, conf_space="", ref_opts=options, sf_opts={'NUM_ROOTS': 4} )
...
def test_2():
    ...
    wfn = fock_ci( 2, 2, n2_3, conf_space="", ref_opts=options, sf_opts={'NUM_ROOTS': 4} )
    for i, true in enumerate(wfn.e):
        assert abs(true - expected[i]) < threshold
...
def test_3():
    ...
    wfn = fock_ci( 2, 2, o2, conf_space="", ref_opts=options, sf_opts={'NUM_ROOTS': 2} )
    for i, true in enumerate(wfn.e):
        assert abs(true - expected[i]) < threshold
