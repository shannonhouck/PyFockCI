...
def test_1():
    ...
    expected = sf_cas_ref( -1, 4, n2_7, conf_space="", add_opts=options )
    wfn = fock_ci( 1, 2, n2_7, conf_space="", ref_opts=options )
...
def test_2():
    ...
    expected = sf_cas_ref( -1, 2, n2_3, conf_space="", add_opts=options )
    wfn = fock_ci( 1, 2, n2_3, conf_space="", ref_opts=options )
    assert abs(wfn.e[0] - expected) < threshold
...
def test_3():
    ...
    wfn = fock_ci( 1, 2, o2, conf_space="", ref_opts=options, sf_opts={'NUM_ROOTS': 2} )
    assert abs(wfn.e[0] - expected) < threshold
