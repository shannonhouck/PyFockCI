...
def test_1():
    ...
    expected = [-108.607407879403027, -108.570261189159140, -108.570261189158586, -108.537213794308300]
    wfn = sf_ip_ea.fock_ci( 0, 1, n2_7, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 4} )
...
def test_2():
    ...
    expected = [-108.630935262330411, -108.630935262330311, -108.592834223639969, -108.480886163796654]
    wfn = sf_ip_ea.fock_ci( 0, 1, n2_3, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 4} )
    for i, true in enumerate(wfn.e):
        assert abs(true - expected[i]) < threshold
...
def test_3():
    ...
    expected = [-149.491801512701898, -149.491801512701755]
    wfn = sf_ip_ea.fock_ci( 0, 1, o2, conf_space="h", ref_opts=options, sf_opts={'NUM_ROOTS': 2} )
    for i, true in enumerate(wfn.e):
        assert abs(true - expected[i]) < threshold
