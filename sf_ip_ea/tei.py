...
class TEIDFPsi4(TEIDFBase):
    ...
    def __init__(self, C, basis, aux, ras1, ras2, ras3, conf_space):
        ...
        mints = psi4.core.MintsHelper(basis)
        df_mints = psi4.core.DFHelper(basis, aux)
        df_mints.set_memory(int(psi4.core.get_memory() * 0.8))
        df_mints.set_method("STORE")
        df_mints.set_nthreads(psi4.core.get_num_threads())
        df_mints.initialize()
        nbf_aux = aux.nbf()
        # set up C
        ...
        # Bnm notation, where n and m indicate RAS space (1/2/3)
        if(conf_space == ""):
            C_ras2 = psi4.core.Matrix.from_array(C[:, ras1:ras1+ras2])
            df_mints.add_space("p", C_ras2)
            df_mints.add_space("q", C_ras2)
            df_mints.add_transformation("Qpq", "p", "q", "Qpq")
            df_mints.transform()
            self.B22 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qpq"))
        if(conf_space == "h"):
            C_ras1 = psi4.core.Matrix.from_array(C[:, 0:ras1])
            C_ras2 = psi4.core.Matrix.from_array(C[:, ras1:ras1+ras2])
            df_mints.add_space("i", C_ras1)
            df_mints.add_space("j", C_ras1)
            df_mints.add_space("p", C_ras2)
            df_mints.add_space("q", C_ras2)
            df_mints.add_transformation("Qij", "i", "j", "Qpq")
            df_mints.add_transformation("Qip", "i", "p", "Qpq")
            df_mints.add_transformation("Qpi", "p", "i", "Qpq")
            df_mints.add_transformation("Qpq", "p", "q", "Qpq")
            df_mints.transform()
            self.B11 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qij"))
            self.B12 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qip"))
            self.B21 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qpi"))
            self.B22 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qpq"))
        if(conf_space == "p"):
            C_ras2 = psi4.core.Matrix.from_array(C[:, ras1:ras1+ras2])
            C_ras3 = psi4.core.Matrix.from_array(C[:, ras1+ras2:])
            df_mints.add_space("p", C_ras2)
            df_mints.add_space("q", C_ras2)
            df_mints.add_space("a", C_ras3)
            df_mints.add_space("b", C_ras3)
            df_mints.add_transformation("Qpq", "p", "q", "Qpq")
            df_mints.add_transformation("Qpa", "p", "a", "Qpq")
            df_mints.add_transformation("Qap", "a", "p", "Qpq")
            df_mints.add_transformation("Qab", "a", "b", "Qpq")
            df_mints.transform()
            self.B22 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qpq"))
            self.B23 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qpa"))
            self.B32 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qap"))
            self.B33 = psi4.core.Matrix.to_array(df_mints.get_tensor("Qab"))
        if(conf_space == "h,p"):
            C_ras1 = psi4.core.Matrix.from_array(C[:, 0:ras1])
            C_ras2 = psi4.core.Matrix.from_array(C[:, ras1:ras1+ras2])
            C_ras3 = psi4.core.Matrix.from_array(C[:, ras1+ras2:])
