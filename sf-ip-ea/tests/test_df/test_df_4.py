...
def test_1():
    ...
    expected = sf_cas_ref( 1, 4, n2_7, conf_space="h", add_opts=options )
    e = fock_ci( 2, 1, n2_7, conf_space="h", ref_opts=options, sf_opts={'integral_type': 'DF'} )
...
def test_2():
    ...
    e = fock_ci( 2, 1, n2_3, conf_space="h", ref_opts=options, sf_opts={'integral_type': 'DF'} )
    assert abs(e[0] - expected) < threshold
...
def test_3():
    ...
    expected = sf_cas_ref( 1, 2, o2, conf_space="h", add_opts=options )
    e = fock_ci( 2, 1, o2, conf_space="h", ref_opts=options, sf_opts={'integral_type': 'DF', 'NUM_ROOTS': 2} )
    assert abs(e[0] - expected) < threshold
